...
partial class PlayerInventory : IBaseInventory
{
	...
	// 2
	public BaseWeapon MeleeWeapon { get; set; }

	// 3-♾️
	public List<BaseWeapon> Gadgets { get; set; } = new();
	...
	{
		if ( ent is BaseWeapon bc && bc.CanCarry( Owner ) )
			return true;
	...
	public bool Add( Entity ent, bool makeactive = false )
	{
		...
		var carriable = ent as BaseWeapon;
	...
	{
		if ( child is not BaseWeapon weapon )
			return;
